    ...
    if (result.success && result.data) {
      ...
      if (foundTask) {
        ...
        if (foundTask.workType === testTask.workType) {
          ...
          error(`  ✗ workType deserialization failed (expected: ${testTask.workType}, got: ${foundTask.workType})`);
          testsFailed++;
        }
  ...
  try {
    ...
    if (result.success && result.data) {
      ...
      if (result.data.activities && result.data.activities.length === 4) {
        ...
        error(`  ✗ activities update failed (expected 4, got ${result.data.activities?.length || 0})`);
        testsFailed++;
      }
