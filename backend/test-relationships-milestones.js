...
async function test1_CreateTasksWithRelationships() {
  ...
  try {
    ...
    const relCount = blueClient.localRelationships.filter(
      (r) => r.fromTaskId === taskA.data.id
    ).length;
    if (relCount === 0) {
    ...
    info(
      `  Relationships: ${JSON.stringify(
        blueClient.localRelationships.filter((r) => r.fromTaskId === taskA.data.id),
        null,
        2
      )}`
    );
...
async function test2_CreateTaskWithMilestones(taskIds) {
  ...
  try {
    ...
    const linkA = blueClient.localMilestoneLinks.find(
      (l) => l.taskId === taskIds.taskAId && l.milestoneId === milestoneId
    );
    const linkB = blueClient.localMilestoneLinks.find(
      (l) => l.taskId === taskIds.taskBId && l.milestoneId === milestoneId
    );
...
async function test4_LocalPersistence() {
  ...
  try {
    ...
    try {
      ...
      const testTasks =
        parsed.tasks?.filter((t) => t.title && t.title.startsWith('TEST_REL_')) || [];
      ...
      const testRels =
        parsed.relationships?.filter((r) => testTasks.some((t) => t.id === r.fromTaskId)) || [];
