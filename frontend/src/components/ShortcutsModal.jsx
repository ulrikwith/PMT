    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 space-y-4">
          <ShortcutRow keys={['Cmd', 'K']} description="Focus Search" icon={<Search size={14} />} />
          <ShortcutRow
            keys={['Cmd', '/']}
    ...
    <div className="flex items-center justify-between group">
      ...
      <div className="flex gap-1.5">
        {keys.map((k) => (
          <kbd
            key={k}
            className="min-w-[24px] px-2 py-1 text-xs font-semibold text-slate-400 bg-slate-800 border-b-2 border-slate-700 rounded-md"
          >
            {k}
          </kbd>
        ))}
