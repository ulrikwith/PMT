...
function WorkNode({ data, selected }) {
  ...
  return (
    ...
    >
      ...
      {!isEmpty && <div className={`h-1.5 w-full ${getDimensionColor(data.dimension).bg}`} />}
              ...
              (isComplete ? (
                ...
                <div className="flex flex-col items-end">
                  <span className="text-[10px] font-bold text-slate-500 uppercase">
                    {Math.round(
                      (data.activities?.filter((a) => a.status === 'done').length /
                  ...
                  <div className="w-12 h-1 bg-slate-800 rounded-full mt-0.5 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(data.activities?.filter((a) => a.status === 'done').length / (data.activities?.length || 1)) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div className={`grid ${gridCols} gap-2`}>
                  {data.activities.map((act, i) => (
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-medium truncate ${act.status === 'done' ? 'line-through opacity-50' : ''}`}
                      >
                        {act.title}
                      </span>
                      {act.timeEstimate && (
