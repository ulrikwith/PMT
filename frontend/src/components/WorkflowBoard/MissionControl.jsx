...
export default function MissionControl({ dimension }) {
  ...
  return (
    <div className="fixed bottom-24 left-6 z-50">
      {isOpen ? (
        <div className="glass-panel rounded-2xl p-6 w-96 shadow-2xl border-purple-500/20 animate-fade-in">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`p-2 rounded-lg bg-${config.color}-500/10 text-${config.color}-500 border border-${config.color}-500/20`}>
                <Compass size={18} />
              </div>
              <h3 className="text-lg font-semibold text-white">{config.label} Vision</h3>
            </div>
            <button
        ...
        <button
          ...
          className="px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold rounded-full shadow-2xl shadow-purple-500/40 hover:shadow-purple-500/60 hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 group border border-white/10 min-w-[180px]"
        >
          <Compass
            size={24}
            className="text-purple-200 group-hover:text-white transition-colors"
          />
