...
export default function TimelineView() {
  ...
  if (loading && tasks.length === 0)
    return <div className="text-slate-500">Loading timeline...</div>;
  if (error)
    return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <span
            className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${getColorClasses(dimension.color)}`}
          >
            {tasks.length} items
          </span>
        </div>
        ...
        <div className="px-6 pb-6 space-y-3 border-t border-white/5 animate-fade-in pt-4">
          {tasks.length === 0 ? (
            <div className="text-slate-500 text-sm italic py-2">
              No tasks scheduled for this dimension.
            </div>
          ) : (
  ...
  return (
    <div
      onClick={() => {
        ...
        const dimTag = task.tags?.find((t) =>
          ['content', 'practice', 'community', 'marketing', 'admin'].includes(t.toLowerCase())
        );
        const query = dimTag
          ? `?dimension=${dimTag.toLowerCase()}&edit=${task.id}`
          : `?edit=${task.id}`;
    ...
    >
      <div className="flex-1">
        <div className="flex items-center gap-2 mb-1">
          <h4
            className={`font-medium text-sm ${isDone ? 'text-slate-500 line-through' : 'text-slate-200'}`}
          >
            {task.title}
          </h4>
          {isDone && (
            <span className="text-[10px] bg-green-500/20 text-green-500 px-1.5 rounded">Done</span>
          )}
        </div>
        ...
        <div className="flex gap-2">
          {task.tags &&
            task.tags.slice(0, 3).map((tag) => (
              <span
                key={tag}
                className="text-[10px] px-1.5 py-0.5 bg-slate-800 text-slate-400 rounded flex items-center gap-1"
              >
                <Hash size={10} /> {tag}
              </span>
            ))}
