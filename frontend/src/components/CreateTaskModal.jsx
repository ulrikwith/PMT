      ...
      for (const dim of DIMENSIONS) {
        ...
        for (const child of dim.elements) {
          ...
          if (
            tags.some(
              (t) =>
                t.toLowerCase() === child.id.toLowerCase() ||
                t.toLowerCase() === child.label.toLowerCase()
            )
          ) {
            foundDim = dim.id;
        ...
        <div className="grid grid-cols-5 gap-3">
          {DIMENSIONS.map((dim) => (
            ...
            >
              ...
              <span
                className={`text-xs font-medium ${selectedDimension === dim.id ? 'text-white' : 'text-slate-400'}`}
              >
                {dim.label}
              </span>
            </button>
    ...
    <div className="space-y-6 animate-in slide-in-from-right-4 duration-200">
      ...
      <div className="space-y-3">
        {activities.map((act, idx) => (
          <div
            key={idx}
            className="glass-panel p-3 rounded-lg flex justify-between items-center border border-white/5"
          >
      ...
      <div className="w-full max-w-2xl bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-8 py-6 border-t border-white/5 bg-slate-900/50 flex justify-between">
          <button
            onClick={handleBack}
            disabled={step === 1}
            className={`px-6 py-2 rounded-lg font-medium ${step === 1 ? 'opacity-0' : 'text-slate-400 hover:text-white'}`}
          >
            Back
          </button>
          ...
          {step < 5 ? (
            <button
              onClick={handleNext}
              disabled={step === 1 && !selectedDimension}
            ...
            <button
              ...
              className="px-6 py-2 bg-emerald-600 text-white rounded-lg font-medium hover:bg-emerald-500 disabled:opacity-50 shadow-lg shadow-emerald-500/20"
            >
