...
const TaskCard = React.memo(function TaskCard({ task, onUpdate, onDelete }) {
  ...
  const taskRelationships = useMemo(() => {
    return allRelationships.filter((r) => r.fromTaskId === task.id || r.toTaskId === task.id);
  ...
  const getTagStyle = (tagName) => {
    ...
    if (lower.includes('substack'))
      return base + 'bg-orange-500/10 text-orange-500 border-orange-500/20';
    ...
    >
      <div className="flex justify-between items-start gap-4">
          ...
          {task.activities && task.activities.length > 0 && (
            <div className="mb-4 max-w-xs">
              <div className="flex justify-between text-[10px] text-slate-500 mb-1 font-medium">
                ...
                <span>
                  {Math.round(
                    (task.activities.filter((a) => a.status === 'done').length /
        ...
        <div className="flex flex-col items-end gap-2">
          <div
            className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-semibold uppercase tracking-wider ${getStatusStyle(task.status)}`}
          >
          ...
          <div className="flex gap-1">
            ...
            <button
              onClick={() =>
                onUpdate(task.id, { status: task.status === 'Done' ? 'In Progress' : 'Done' })
              }
        ...
        <div className="mt-6 pt-6 border-t border-slate-700/50 animate-fade-in">
          <div className="flex flex-col md:flex-row gap-6">
            ...
            <div className="w-full md:w-1/3 space-y-4">
              <div className="text-xs text-slate-500">
                ...
                {task.resources && task.resources.timeEstimate && (
                  <div className="mt-2 text-blue-400">Est: {task.resources.timeEstimate}h</div>
                )}
              </div>
            </div>
            ...
            <div className="w-full md:w-2/3">
              ...
              <RelationshipMap taskId={task.id} relationships={taskRelationships} />
            </div>
          </div>
        </div>
      )}
